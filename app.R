...
# In future, may use all_games, but the extra columns were unhelpful in the
...
ui <- navbarPage(
                 ...
                 sidebarLayout(
                     sidebarPanel(
                         radioButtons(
                             ...
                             choices = list(
                                 "Elo Rating Over Time" = "smoothest_elo_plot.gif",
                 ...
                 sidebarLayout(
                     sidebarPanel(
                         radioButtons(
                             ...
                             choices = list(
                                 "Opening-Specific Model (Default)" = "stat_sig_openings.png",
                     ...
                     mainPanel(
                         ...
                         conditionalPanel(
                             ...
                             h3("Interpretation"),
                             p(
                                 "The above visualization reflects the result of a linear
                           ...
                           study."
                             ...
                             h3("Limitations"),
                             p(
                                 "As a few caveats, my method here was somewhat restricted
                           ...
                           yielded 41 ECO codes, for a total of 83 parameters."
                             ...
                             p(
                                 "Another important caveat is that this model does not
                           ...
                           rather than illuminate, my understanding."
                             ...
                             p(
                                 "An additional shortcoming is that this model uses
                             ...
                             p(
                                 "The issue of having to severely prune the
                           ...
                           which are addressed in that section."
                             ...
                             h3("Formula"),
                             p(
                                 "For the curious, the mathematical formula for the
                               model is something like"
                             ),
                             
                             withMathJax(),
                             helpText(
                                 "$$ outcome \\_ i = \\beta_0 rating \\_ diff_i
                             ...
                             p(
                                 "where outcome_i is 0 for a loss, 0.5 for a draw,
                         ...
                         conditionalPanel(
                             ...
                             h3("Interpretation"),
                             p(
                                 "Instead of trying to find performance differences
                           ...
                           confidence from my average score. (Note that the groups
                             ...
                             p(
                                 "The first is the group A50-A79 when I play with the
                           ...
                           more often!"
                             ...
                             p(
                                 "The second is the group C40-C59 when I play with the
                           ...
                           small magnitude, from my average score."
                             ...
                             p(
                                 "Thirdly, I perform slightly lower than average when
                           ...
                           disadvantage."
                             ...
                             p(
                                 "Finally, I perform poorly in the group B10-B19 with the
                           ...
                           of play in my 27 games in this group."
                             ...
                             h3("Caveats"),
                             p(
                                 "As mentioned, this model also has its limitations.
    ...
    tabPanel(
        ...
        p(
            "The famous American chess genius Bobby Fischer was well-known for
        ...
        h3("Auxiliary Info"),
        p(
            "The R package 'bigchess' allowed me to download all of my games
        ...
        h3("About Me"),
        p(
            "Hello! My name is Nick Brinkmann and I study Applied Mathematics
             in the Class of 2023 at Harvard College.
             You can reach me at nickbrinkmann@college.harvard.edu."
        ),
        p(
            "I have been a chess enthusiast since I learned the game at the age of 6.
        ...
        p(
            "See my Github repo ",
            tags$a(href = "https://github.com/nick-brinkmann/best-to-test",
                   "here"),
            "."
        ),
        p(
            "See my online profiles on ",
    ...
    tabPanel(
        ...
        p(
            "For those who are unfamiliar with chess, I will give some brief
               background so you are able to understand the project."
        ),
        h3("Notation"),
        p(
            "In chess, moves are written using 'Algebraic Notation', where
        ...
        p(
            "Chess players like to categorize games they play based on any number
        ...
        h3("Time Control"),
        p(
            "Almost all games played online have a time restriction on the players.
        ...
        h3("Rating"),
        p(
            "Each player is given a rating: a single number which measures their
        ...
        p(
            "A beginner player typically has a rating of under 1000, while the
        ...
        p(
            "A common metric for measuring how well someone played in a tournament
        ...
        withMathJax(),
        helpText(
            "$$performance \\_rating = \\frac{sum(opponent \\_ rating)
    ...
    tabPanel(
        ...
        fluidPage(
            ...
            p(
                "In the extremely unlikely case that you have any interest in browsing
...
server <- function(input, output) {
    ...
    # PLAY THROUGH GAMES SECTION BELOW
    
    # The reactiveValues part is absolutely essential! Everything breaks otherwise.
    # Note that it is impossible to change the value of an action button, which
    # is why this is necessary.
    
    move_counter <- reactiveValues(countervalue = 0)
    
    # Increments the counter. This then makes a move in the renderChessboardjs
    # function below.
    ...
    # Gives move counter and says whose move it is.
    ...
    # Tells you which player is playing White
    
    output$white <- renderText({
        paste(
            all_games_dense$White[ifelse(is.na(input$gameId), 1, input$gameId)],
            ", with a rating of",
            all_games_dense$WhiteElo[ifelse(is.na(input$gameId), 1, input$gameId)],
            ", is playing White."
        )
    })
    
    # Tells you which player is playing Black
    
    output$black <- renderText({
        paste(
            all_games_dense$Black[ifelse(is.na(input$gameId), 1, input$gameId)],
            ", with a rating of",
            all_games_dense$BlackElo[ifelse(is.na(input$gameId), 1, input$gameId)],
            ", is playing Black."
        )
    })
    
    # Resets board when new Game ID given
    ...
    # Renders the chessboard and makes moves when the 'next move' button is pressed.
    
    output$board <- renderChessboardjs({
        tbl_mvs <- all_games_dense$Movetext[ifelse(is.na(input$gameId), 1, input$gameId)] %>%
            extract_moves(N = 120) %>%
